...
func (s *Service) PlanVisit(ctx context.Context, u domain.User, caseID, assigneeID string, when time.Time, key, requestID string) (domain.Visit, error) {
	...
	if key != "" {
		if raw, err := s.Idempotency.Find(ctx, key, u.ID, idempotencyLookupOperation(), time.Now().UTC()); err == nil {
			var v domain.Visit
	...
	err = s.DB.WithTx(ctx, func(tx *sql.Tx) error {
		...
		if _, e := tx.ExecContext(ctx, `INSERT INTO visits(id,case_id,household_id,assignee_id,status,scheduled_for,version,created_at) VALUES(?,?,?,?,?,?,?,?)`, v.ID, v.CaseID, v.HouseholdID, v.AssigneeID, v.Status, v.ScheduledFor.Format(timeLayout), v.Version, stamp); e != nil {
...
func idempotencyLookupOperation() string {
	return "plan_visit_retry"
}
