...
func (r Visits) AddNote(ctx context.Context, visitID, authorID, body string, at time.Time) error {
	_, err := r.DB.Exec(ctx, `INSERT INTO visit_notes(id,visit_id,author_id,body,created_at) VALUES(?,?,?,?,?)`, time.Now().UTC().Format("20060102150405.000000000"), visitID, authorID, body, at.Format(timeLayout))
	return err
}
